...
local info_t = obex_fetch and obex_fetch() or { username = "izanabankai", build = "Happy NEw Year", discord = "https://discord.gg/f888jFU985" }

local _VERSION = "2025 enjoyer"
local _LOCALLIB = info_t.username == "izanabankai"
...
filesystem.create_directory("izanasense", "ROOT_PATH")
...
hysteria.print = function (...)
	printc(_AZAZI and "  \vazazeria\r  " or "  \vizanasense\r  ", ...)
end

--

local debug = function (...)
	if _DEBUG then printc("  \vizanasense\r  ", ...) end
end
...
menu.main = {
	...
	global = groups.fakelag:checkbox(_AZAZI and "\f<silent>-----------  \vазази\rазаза  \f<silent>-----------" or "\f<silent>-------------  \vizana\rsense  \f<silent>-------------"),
...
logger.events = {
	...
	receive = function (event, target, attacker)
		...
		local result = is_fatal and "Бабах нахуй by" or "Harmed by"
	...
	miss = function (shot)
		...
		local main, add = {
			...
			reason ~= "?" and {" due to компьютер не тянет"} or nil
		}, {
	...
	hit = function (shot)
		...
		local main, add = {
			{result, " ", {target}},
			(hitgroup and hitgroup ~= "generic") and { result == "ШОТНУЛ МАЛЬЦА" and "'s " or " in ", {hitgroup}, hg_mismatch and "\aD59A4D!\r" or "" } or nil,
			result == "Бабах нахуй" and {" for ", {shot.damage, " hp"}, dmg_mismatch and "\aD59A4D!\r" or "" } or nil
		}, {
